...
class DataProcessor:
    ...
    def create_sliding_windows(self, sequence, label):
        """Split the given sequence into overlapping windows with stride and store fixed-size features."""
        windows = []
        ...
        for start in range(0, seq_len, self.stride):
            ...
            # If shorter than window_size, pad with zeros
            ...
            # Flatten the window sequence to create a feature vector
            window_feature = window_seq.flatten()
            windows.append((window_feature, label))
    ...
    def create_sequences(self):
        ...
        cols = [self.traj_id_column] + self.feature_columns + [self.target_column]
        for chunk in tqdm(pd.read_csv(self.data_path, usecols=cols, chunksize=self.chunksize), desc="Processing chunks"):
            train_chunk = chunk[chunk[self.traj_id_column].isin(self.train_ids)].copy()
            ...
            for split, df in zip(['train', 'val', 'test'], [train_chunk, val_chunk, test_chunk]):
                ...
                for _, group in grouped:
                    ...
                    if split == 'train':
                        ...
                        self.train_labels.extend([w[1] for w in windows])
                    elif split == 'val':
                        self.val_sequences.extend([w[0] for w in windows])
                        self.val_labels.extend([w[1] for w in windows])
                    else:
                        self.test_sequences.extend([w[0] for w in windows])
                        self.test_labels.extend([w[1] for w in windows])
    ...
    def save_preprocessors(self, scaler_path, label_encoder_path):
        """Save the fitted scaler and label encoder to disk."""
    ...
    def get_data_splits(self):
        ...
        X_train = np.array(self.train_sequences)
        y_train = np.array(self.train_labels)

        X_val = np.array(self.val_sequences)
        y_val = np.array(self.val_labels)

        X_test = np.array(self.test_sequences)
